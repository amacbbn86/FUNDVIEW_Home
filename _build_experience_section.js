/**
 * Build a self-contained experience-section.html from home4.html pieces:
 * CSS (experience block) + section markup + experience JS + asset links.
 */
const fs = require("fs");
const path = require("path");

const ROOT = __dirname;
const home4 = fs.readFileSync(path.join(ROOT, "home4.html"), "utf8");
const lines = home4.split(/\r?\n/);
...
const cssEnd = findIndex(
  (l) => l.includes(".solution_delivers {"),
  cssStart + 1
);
...
// Include from the experience comment through the blank line before solution_delivers
let cssLast = cssEnd - 1;
while (cssLast > cssStart && lines[cssLast].trim() === "") cssLast--;

const secStart = findIndex((l) =>
  l.includes('id="experience_fundview"')
);
const secEndComment = findIndex(
  (l) => l.includes("Experience section end"),
  secStart + 1
);
// Include the closing </section> after the comment if present
...
<!--
  Experience FUNDVIEW — standalone working copy for editing.
  Source of truth while iterating: this file.
  When ready, copy the <section id="experience_fundview"> … </section> block
  back into home4.html (and keep CSS/JS in sync if those changed).

  Preview: http://localhost:5500/experience-section.html
  Built from home4.html experience CSS + markup + scripts.
-->
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Experience FUNDVIEW — section preview</title>
...
const outPath = path.join(ROOT, "experience-section.html");
...
console.log("Has zoomClip CSS:", out.includes("#zoomClip"));
